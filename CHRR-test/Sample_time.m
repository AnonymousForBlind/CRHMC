function P = Sample_time(N,skip,P,E,eps,flags,start_pt)
...
samples = zeros(size(x, 1), N);
for i = 1:N
    if (toc(startTime) > maxTime)
        fprintf("Time over: 24hr passed\n")
...
P.attempts = size(samples, 2);
samples=P.N*samples + P.p_shift;
samples=P.T*samples + P.y;
P.sampleTime = toc(startTime);
